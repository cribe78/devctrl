...
DevCtrl.DataService.factory=["$window","$http","$mdToast","$timeout","$q","socketFactory","$mdDialog","$location",function(b,c,a,d,e,f,k,l){var h={user:{username:null,admin:!1,client_id:0},applog:[]},p={},q=!1,r=c.get("schema.php").success(function(a){angular.isDefined(a.schema)&&angular.forEach(a.schema,function(a,b){var u=g.getSchema(b);angular.merge(u,a);angular.isDefined(u.foreign_keys)&&angular.forEach(u.foreign_keys,function(a,b){u.referenced[a]=b})});q=!0}).error(function(a){g.errorToast(a)}),t=
io(l.protocol()+"://"+l.host()),m=f({ioSocket:t}),v={},n={};f={editEnabled:!0};"undefined"!==typeof b.localStorage&&(t=b.localStorage.config,angular.isString(t)?f=JSON.parse(t):b.localStorage.config=JSON.stringify(f));var g={config:f,messenger:m,dataModel:h,schema:p,addRow:function(a,b){var c={};c[a.tableName]=[a.fields];m.emit("add-data",c,function(c){if(c.error)g.errorToast(data);else{var d=Object.keys(c.data.add[a.tableName])[0];console.log("new record "+d+"added to "+a.tableName);g.loadData(c.data);
c=h[a.tableName].indexed[d];angular.forEach(a,function(b,c){"tableName"!=c&&(a[c]=null)});angular.isFunction(b)&&b(c)}})},dialogClose:function(){k.hide()},deleteRow:function(a){var b="data.php/"+a.tableName+"/"+a.id,d=!1;angular.forEach(a.referenced,function(a,b){0<Object.keys(a).length&&(d=b)});d?g.errorToast({error:"Cannot delete "+a.tableName+" record due to foreign key constraint on "+d}):c.delete(b).success(function(a){g.loadData(a)}).error(function(a){g.errorToast(a)})},editEnum:function(a,
...
clickOutsideToClose:!0,hasBackdrop:!1})},editRecordClose:function(){k.hide()},errorToast:function(b){var c="An unknown error has occured";angular.isDefined(b.error)&&(c=b.error);a.show({templateUrl:"ng/error-toast.html",locals:{message:c},controllerAs:"toast",controller:"RoomsCtrl",bindToController:!0,position:"top right",hideDelay:3E3})},getAdminAuth:function(a){var b="admin_auth.php";a&&(b="admin_auth.php?logon=1&location="+l.path());return c.get(b).then(function(a){angular.isDefined(a.data.user)?
angular.merge(h.user,a.data.user):console.log("admin_auth did not return an admin status")},function(b){"401"==b.status&&(a&&angular.isDefined(b.data.location)?window.location=b.data.location:(angular.isDefined(b.data.user)&&angular.merge(h.user,b.data.user),g.errorToast(b.data)))})},getLog:function(){return c.get("log.php").then(function(a){angular.isDefined(a.data.applog)&&(h.applog.length=0,angular.merge(h.applog,a.data.applog))})},getMData:function(a,b){var c={table:a,params:b};g.getMProm=e(function(a,
b){m.emit("get-data",c,function(b){console.log("data received:"+b);g.loadData(b);a(!0)})});return g.getMProm},getNewRowRef:function(a){var b={id:"0",referenced:{},tableName:a,fields:{}};a=g.getSchema(a);angular.forEach(a.fields,function(a,c){b.fields[a.name]=""});return b},getRowRef:function(a,b){if(!angular.isDefined(a))return console.error("error looking up record for undefined table"),{};if(!angular.isDefined(b)||null===b)return console.error("error looking up %s record for undefined key",a),{};
var c=this.getTableRef(a);angular.isDefined(c.indexed[b])||(c.indexed[b]={fields:{},foreign:{},id:b,loaded:!1,referenced:{},tableName:a},c.listed.push(c.indexed[b]));return c.indexed[b]},getMenu:function(){angular.isDefined(h.menu)||(h.menu={items:{}},c.get("menu.php").success(function(a){angular.isDefined(a.menu.items)?(console.log("test/menu.php loaded"),h.menu.items=a.menu.items):console.log("test/menu.php did not return a valid menu object")}).error(function(a){g.errorToast(a)}));return h.menu},
getSchema:function(a){p.loaded||angular.isDefined(p[a])||(p[a]={referenced:{},foreign_keys:{}});return p[a]},getSchemaPromise:function(){if(!q)return r},getTable:function(a){angular.isDefined(h[a])?angular.isDefined(n[a])||(n[a]=this.getTablePromise(a)):(this.getTableRef(a),n[a]=this.getTablePromise(a));return h[a]},getTablePromise:function(a){if(angular.isDefined(n[a]))return n[a];if(angular.isDefined(a))return n[a]=g.getMData(a,{}).then(function(){return q?h[a]:r.then(function(){return h[a]})},
function(){g.errorToast("getMData "+a+" problem")}),n[a];console.error("error: attempt to fetch undefined table!")},getTableRef:function(a){angular.isDefined(h[a])||(h[a]={listed:[],indexed:{},loaded:!1});return h[a]},guid:function(){return"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(a){var b=16*Math.random()|0;return("x"==a?b:b&3|8).toString(16)})},isAdminAuthorized:function(){return h.user.admin},loadControlUpdates:function(a){var b;for(b=0;b<a.length;b++){var c=a[b];if("observed"==
c.status||"executed"==c.status)g.getRowRef("controls",c.control_id).fields.value=c.value}},loadData:function(a){if(q)g.loadDataKernel(a);else return r=r.then(function(){g.loadDataKernel(a)})},loadDataKernel:function(a){angular.isDefined(a.update)&&(angular.isDefined(a.add)?angular.merge(a.add,a.update):a.add=a.update);angular.isDefined(a.add)&&angular.forEach(a.add,function(a,b){var c=g.getSchema(b).foreign_keys;angular.forEach(a,function(a,d){var e=g.getRowRef(b,d);angular.merge(e.fields,a);e.loaded=
!0;angular.forEach(c,function(a,c){if(angular.isDefined(e.fields[c])&&null!==e.fields[c]){var d=g.getRowRef(a,e.fields[c]);angular.isDefined(d.referenced[b])||(d.referenced[b]={});d.referenced[b][e.id]=e;e.foreign[a]=d;e.foreign[c]=d}else e.foreign[a]=null,e.foreign[c]=null})})});angular.isDefined(a.delete)&&angular.forEach(a.delete,function(a,b){angular.forEach(a,function(a,c){angular.forEach(h[b].indexed[c].foreign,function(a,d){angular.isDefined(a.referenced[b][c])&&delete a.referenced[b][c]});
delete h[b].indexed[c]});h[b].listed.length=0;angular.forEach(h[b].indexed,function(a,c){h[b].listed.push(a)})})},revokeAdminAuth:function(){c.get("admin_auth_revoke.php").then(function(a){angular.isDefined(a.data.user)?angular.merge(h.user,a.data.user):console.log("revoke admin_auth did not return user info")},function(a){g.errorToast(a.data)})},showControlLog:function(a,b){g.getMData("control_log",{control_id:b.id}).then(function(){k.show({targetEvent:a,locals:{ctrl:b},controller:DevCtrl.CtrlLog.Ctrl,
controllerAs:"ctrlLog",bindToController:!0,templateUrl:"ng/ctrl-log.html",clickOutsideToClose:!0,hasBackdrop:!1})})},updateConfig:function(){"undefined"!==typeof b.localStorage&&(b.localStorage.config=JSON.stringify(g.config))},updateControlValue:function(a){angular.isDefined(v[a.id])&&d.cancel(v[a.id]);v[a.id]=d(function(a,b){var c=[{_id:b.guid(),control_id:a.id,value:a.fields.value,type:"user",status:"requested",source:b.dataModel.client_id}];m.emit("control-updates",c)},200,!0,a,g)},updateRow:function(a){m.emit("update-data",
{table:a.tableName,_id:a.id,set:a.fields},function(a){console.log("data updated:"+a);g.loadData(a)})}};m.on("control-data",function(a){g.loadData(a)});m.on("control-updates",function(a){g.loadControlUpdates(a)});m.on("log-data",function(a){g.dataModel.applog.push(a)});return g}];DevCtrl.Slider2d={};
...
DevCtrl.Table.Ctrl).controller("RecordCtrl",DevCtrl.Record.Ctrl).controller("RoomCtrl",DevCtrl.Room.Ctrl).controller("RoomsCtrl",DevCtrl.Rooms.Ctrl).config(DevCtrl.stateConfig).run(["$rootScope",function(b){b.$on("$stateChangeStart",function(b,a,d,e,f){});b.$on("$stateChangeError",function(b,a,d,e,f){console.log("$stateChangeError - fired when an error occurs during transition.");console.log(arguments)});b.$on("$stateChangeSuccess",function(b,a,d,e,f){});b.$on("$viewContentLoaded",function(b){});
b.$on("$stateNotFound",function(b,a,d,e){console.log("$stateNotFound "+a.to+"  - fired when a state cannot be found by its name.");console.log(a,d,e)})}]);
