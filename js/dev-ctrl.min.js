...
function(b){a.editRecord(b,0,"control_endpoints")};this.addEndpointType=function(b){a.editRecord(b,0,"endpoint_types")};this.dataModel=a.dataModel;this.title="DevCtrl";this.top=!0}];DevCtrl.AdminOnly={};DevCtrl.AdminOnly.Directive=["$compile","DataService",function(d,c){return{restrict:"A",replace:!1,terminal:!0,priority:1E3,link:function(a,b,e){b.removeAttr("devctrl-admin-only");b.attr("ng-if","dataServiceConfig.editEnabled");a.dataServiceConfig=c.config;d(b)(a)}}}];DevCtrl.Menu={};DevCtrl.Menu.Directive=["MenuService","$state",function(d,c){return{scope:!0,bindToController:{},controller:function(a,b){this.service=a},controllerAs:"menu",templateUrl:"ng/menu.html"}}];DevCtrl.DataService={};
DevCtrl.DataService.factory=["$window","$http","$mdToast","$timeout","socketFactory","$mdDialog",function(d,c,a,b,e,h){var f={},k={},l=!1,q=c.get("schema.php").success(function(a){angular.isDefined(a.schema)&&angular.forEach(a.schema,function(a,b){var c=g.getSchema(b);angular.merge(c,a);angular.isDefined(c.foreign_keys)&&angular.forEach(c.foreign_keys,function(a,b){c.referenced[a]=b})});l=!0}).error(function(a){g.errorToast(a)}),m=io("https://devctrl.dwi.ufl.edu/");e=e({ioSocket:m});var p={},n={},
m={editEnabled:!0};if("undefined"!==typeof d.localStorage){var r=d.localStorage.config;angular.isString(r)?m=JSON.parse(r):d.localStorage.config=JSON.stringify(m)}var g={config:m,messenger:e,dataModel:f,schema:k,addRow:function(a){c.post("data.php",a).success(function(b){g.loadData(b);angular.forEach(a,function(b,c){"table"!=c&&(a[c]=null)})}).error(function(a){g.errorToast(a)})},deleteRow:function(a){c.delete("data.php/"+a.tableName+"/"+a.id).success(function(a){g.loadData(a)}).error(function(a){g.errorToast(a)})},
editEnum:function(a,b,c,e){angular.isObject(e)||(e={});h.show({targetEvent:a,locals:{myEnum:b,enumRefRecord:c,options:e},controller:DevCtrl.EnumEditor.Ctrl,controllerAs:"editor",bindToController:!0,templateUrl:"ng/enum-editor.html",clickOutsideToClose:!0,hasBackdrop:!1})},editRecord:function(a,b,c,e){"0"!==b?b=g.getRowRef(c,b):(b=g.getNewRowRef(c),angular.isObject(e)&&angular.merge(b.fields,e));h.show({targetEvent:a,locals:{obj:b},controller:DevCtrl.Record.Ctrl,controllerAs:"record",bindToController:!0,
templateUrl:"ng/record.html",clickOutsideToClose:!0,hasBackdrop:!1})},editRecordClose:function(){h.hide()},errorToast:function(b){var c="An unknown error has occured";angular.isDefined(b.error)&&(c=b.error);a.show(a.simple().content(c))},getNewRowRef:function(a){var b={id:"0",referenced:{},tableName:a,fields:{}};a=g.getSchema(a);angular.forEach(a.fields,function(a,c){b.fields[c]=""});return b},getRowRef:function(a,b){if(!angular.isDefined(a))return console.error("error looking up record for undefined table"),
{};if(!angular.isDefined(b)||null===b)return console.error("error looking up %s record for undefined key",a),{};var c=this.getTableRef(a);if(angular.isString(k[a].pk))return angular.isDefined(c.indexed[b])||(c.indexed[b]={fields:{},foreign:{},id:b,loaded:!1,referenced:{},tableName:a},c.listed.push(c.indexed[b])),c.indexed[b];console.error("multi column keys not supported, table %s",a)},getMenu:function(){angular.isDefined(f.menu)||(f.menu={items:{}},c.get("menu.php").success(function(a){angular.isDefined(a.menu.items)?
...
c){var e=g.getRowRef(b,c);angular.merge(e.fields,a)})});angular.isDefined(a.add)&&angular.forEach(a.add,function(a,b){var c=g.getSchema(b),e=c.pk,d=c.foreign_keys;angular.forEach(a,function(a,c){if(angular.isString(e)){var h=g.getRowRef(b,c);angular.merge(h.fields,a);h.loaded=!0;angular.forEach(d,function(a,c){if(null!==h.fields[c]){var e=g.getRowRef(a,h.fields[c]);angular.isDefined(e.referenced[b])||(e.referenced[b]={});e.referenced[b][h.id]=h;h.foreign[a]=e;h.foreign[c]=e}else h.foreign[a]=null,
h.foreign[c]=null})}else console.error("Error loading %s, multi-keyed records not supported",b)})});angular.isDefined(a.delete)&&angular.forEach(a.delete,function(a,b){var c=g.getSchema(b).pk;angular.forEach(a,function(a,e){angular.isString(c)?delete f[b].indexed[e]:angular.forEach(a,function(a,c){delete f[b].indexed[e][c]})});f[b].listed.length=0;angular.forEach(f[b].indexed,function(a,e){angular.isString(c)?f[b].listed.push(a):angular.forEach(a,function(a){f[b].listed.push(a)})})})},updateConfig:function(){"undefined"!==
...
DevCtrl.Rooms.Ctrl).config(DevCtrl.stateConfig).run(["$rootScope",function(d){d.$on("$stateChangeStart",function(c,a,b,d,h){console.log("$stateChangeStart to "+a.to+"- fired when the transition begins. toState,toParams : \n",a,b)});d.$on("$stateChangeError",function(c,a,b,d,h){console.log("$stateChangeError - fired when an error occurs during transition.");console.log(arguments)});d.$on("$stateChangeSuccess",function(c,a,b,d,h){console.log("$stateChangeSuccess to "+a.name+"- fired once the state transition is complete.")});
