...
b=this.config.rooms[this.id];this.getGroups=function(){var k={};angular.forEach(b.groups,function(a,b){k[b]=!0});angular.forEach(a.panels,function(a){angular.isDefined(b.groups[a.fields.grouping])||(b.groups[a.fields.grouping]={opened:!1});k[a.fields.grouping]=!1});angular.forEach(k,function(a,k){a&&delete b.groups[k]});return b.groups};this.panelControls=function(a){if(angular.isDefined(a.referenced.panel_controls))return a.referenced.panel_controls};this.toggleGroup=function(a){a.opened=!a.opened;
...
function(b){a.editRecord(b,0,"control_endpoints")};this.addEndpointType=function(b){a.editRecord(b,0,"endpoint_types")};this.dataModel=a.dataModel;this.title="DevCtrl";this.top=!0;this.adminEnabled=function(){return a.isAdminAuthorized()};this.adminLogin=function(){a.getAdminAuth()}}];DevCtrl.AdminOnly={};DevCtrl.AdminOnly.Directive=["$compile","DataService",function(d,c){return{restrict:"A",replace:!1,terminal:!0,priority:1E3,link:function(a,b,k){var g=!1;"invert"==b.attr("devctrl-admin-only")&&(g=!0);b.removeAttr("devctrl-admin-only");b.attr("ng-if","adminEnabled()");a.adminEnabled=function(){var a=c.isAdminAuthorized()&&c.config.editEnabled;g&&(a=!a);return a};d(b)(a)}}}];DevCtrl.Menu={};DevCtrl.Menu.Directive=["MenuService","$state",function(d,c){return{scope:!0,bindToController:{},controller:function(a,b){this.service=a},controllerAs:"menu",templateUrl:"ng/menu.html"}}];DevCtrl.DataService={};
DevCtrl.DataService.factory=["$window","$http","$mdToast","$timeout","socketFactory","$mdDialog","$location",function(d,c,a,b,k,g,h){var f={},l={},q=!1,r=c.get("schema.php").success(function(a){angular.isDefined(a.schema)&&angular.forEach(a.schema,function(a,b){var n=e.getSchema(b);angular.merge(n,a);angular.isDefined(n.foreign_keys)&&angular.forEach(n.foreign_keys,function(a,b){n.referenced[a]=b})});q=!0}).error(function(a){e.errorToast(a)});h=io("https://devctrl.dwi.ufl.edu/");k=k({ioSocket:h});
var p={},m={},t=!1;h={editEnabled:!0};if("undefined"!==typeof d.localStorage){var u=d.localStorage.config;angular.isString(u)?h=JSON.parse(u):d.localStorage.config=JSON.stringify(h)}var e={config:h,messenger:k,dataModel:f,schema:l,addRow:function(a){c.post("data.php",a).success(function(b){e.loadData(b);angular.forEach(a,function(b,c){"table"!=c&&(a[c]=null)})}).error(function(a){e.errorToast(a)})},deleteRow:function(a){var b="data.php/"+a.tableName+"/"+a.id,d=!1;angular.forEach(a.referenced,function(a,
b){0<Object.keys(a).length&&(d=b)});d?e.errorToast({error:"Cannot delete "+a.tableName+" record due to foreign key constraint on "+d}):c.delete(b).success(function(a){e.loadData(a)}).error(function(a){e.errorToast(a)})},editEnum:function(a,b,c,d){angular.isObject(d)||(d={});g.show({targetEvent:a,locals:{myEnum:b,enumRefRecord:c,options:d},controller:DevCtrl.EnumEditor.Ctrl,controllerAs:"editor",bindToController:!0,templateUrl:"ng/enum-editor.html",clickOutsideToClose:!0,hasBackdrop:!1})},editRecord:function(a,
b,c,d){"0"!==b?b=e.getRowRef(c,b):(b=e.getNewRowRef(c),angular.isObject(d)&&angular.merge(b.fields,d));g.show({targetEvent:a,locals:{obj:b},controller:DevCtrl.Record.Ctrl,controllerAs:"record",bindToController:!0,templateUrl:"ng/record.html",clickOutsideToClose:!0,hasBackdrop:!1})},editRecordClose:function(){g.hide()},errorToast:function(b){var c="An unknown error has occured";angular.isDefined(b.error)&&(c=b.error);a.show({templateUrl:"ng/error-toast.html",locals:{message:c},controllerAs:"toast",
controller:"RoomsCtrl",bindToController:!0,position:"top right",hideDelay:3E3})},getAdminAuth:function(){c.get("admin_auth.php").then(function(a){angular.isDefined(a.data.admin)?t=a.data.admin:console.log("admin_auth did not return an admin status")},function(a){"401"==a.status&&(angular.isDefined(a.data.location)?window.location=a.data.location:e.errorToast(a.data))})},getNewRowRef:function(a){var b={id:"0",referenced:{},tableName:a,fields:{}};a=e.getSchema(a);angular.forEach(a.fields,function(a,
c){b.fields[c]=""});return b},getRowRef:function(a,b){if(!angular.isDefined(a))return console.error("error looking up record for undefined table"),{};if(!angular.isDefined(b)||null===b)return console.error("error looking up %s record for undefined key",a),{};var c=this.getTableRef(a);if(angular.isString(l[a].pk))return angular.isDefined(c.indexed[b])||(c.indexed[b]={fields:{},foreign:{},id:b,loaded:!1,referenced:{},tableName:a},c.listed.push(c.indexed[b])),c.indexed[b];console.error("multi column keys not supported, table %s",
a)},getMenu:function(){angular.isDefined(f.menu)||(f.menu={items:{}},c.get("menu.php").success(function(a){angular.isDefined(a.menu.items)?(console.log("test/menu.php loaded"),f.menu.items=a.menu.items):console.log("test/menu.php did not return a valid menu object")}).error(function(a){e.errorToast(a)}));return f.menu},getSchema:function(a){l.loaded||angular.isDefined(l[a])||(l[a]={referenced:{},foreign_keys:{}});return l[a]},getTable:function(a){angular.isDefined(f[a])?angular.isDefined(m[a])||(m[a]=
this.getTablePromise(a)):(this.getTableRef(a),m[a]=this.getTablePromise(a));return f[a]},getTablePromise:function(a){if(angular.isDefined(m[a]))return m[a];if(angular.isDefined(a))return c.get("data.php?table="+a).success(function(a){e.loadData(a)}).error(function(a){e.errorToast(a)});console.error("error: attempt to fetch undefined table!")},getTableRef:function(a){angular.isDefined(f[a])||(f[a]={listed:[],indexed:{},loaded:!1});return f[a]},isAdminAuthorized:function(){return t},loadData:function(a){q?
e.loadDataKernel(a):r=r.then(function(){e.loadDataKernel(a)})},loadDataKernel:function(a){angular.isDefined(a.update)&&angular.forEach(a.update,function(a,b){angular.forEach(a,function(a,c){var d=e.getRowRef(b,c);angular.merge(d.fields,a)})});angular.isDefined(a.add)&&angular.forEach(a.add,function(a,b){var c=e.getSchema(b),d=c.pk,k=c.foreign_keys;angular.forEach(a,function(a,c){if(angular.isString(d)){var g=e.getRowRef(b,c);angular.merge(g.fields,a);g.loaded=!0;angular.forEach(k,function(a,c){if(null!==
g.fields[c]){var d=e.getRowRef(a,g.fields[c]);angular.isDefined(d.referenced[b])||(d.referenced[b]={});d.referenced[b][g.id]=g;g.foreign[a]=d;g.foreign[c]=d}else g.foreign[a]=null,g.foreign[c]=null})}else console.error("Error loading %s, multi-keyed records not supported",b)})});angular.isDefined(a.delete)&&angular.forEach(a.delete,function(a,b){var c=e.getSchema(b).pk;angular.forEach(a,function(a,c){angular.forEach(f[b].indexed[c].foreign,function(a,d){angular.isDefined(a.referenced[b][c])&&delete a.referenced[b][c]});
delete f[b].indexed[c]});f[b].listed.length=0;angular.forEach(f[b].indexed,function(a,d){angular.isString(c)?f[b].listed.push(a):angular.forEach(a,function(a){f[b].listed.push(a)})})})},updateConfig:function(){"undefined"!==typeof d.localStorage&&(d.localStorage.config=JSON.stringify(e.config))},updateControlValue:function(a){angular.isDefined(p[a.id])&&b.cancel(p[a.id]);p[a.id]=b(function(a,b){var d="control.php/"+a.id;pendingDebounce=!1;c.put(d,a.fields).success(function(a){b.loadData(a)}).error(function(a){b.errorToast(a)})},
200,!0,a,e)},updateRow:function(a){c.put("data.php/"+a.tableName+"/"+a.id,a.fields).success(function(a){e.loadData(a)}).error(function(a){e.errorToast(a)})}};k.on("control-data",function(a){e.loadData(a)});return e}];DevCtrl.Slider2d={};
...
(b.title=b.data.title))});angular.forEach(b,function(b,g){if(""!=b.name){var h=d.get("^",b);if(angular.isDefined(a.items[h.name]))if(angular.isDefined(b.data.listByName)){var f=c.getTable(b.data.listByName).listed;angular.forEach(f,function(a){angular.isDefined(h.substates[a.id])?(h.substates[a.id].params.name=a.fields.name,h.substates[a.id].title=a.fields.name):h.substates[a.id]={name:b.name,params:{name:a.fields.name,id:a.id},title:a.fields.name}})}else a.items[h.name].substates[b.name]=b}});return a.items}};
...
function(){return this.endpoints.indexed};this.controlList={};this.getControls=function(){angular.forEach(a.controls.indexed,function(b){var c=!1,d=!0;if(angular.isArray(a.endpointsSelected)&&0<a.endpointsSelected.length){var d=!1,h=b.fields.control_endpoint_id;angular.forEach(a.endpointsSelected,function(a){a==h&&(c=!0)})}else if(angular.isArray(a.endpointTypesSelected)&&0<a.endpointTypesSelected.length){var d=!1,f=b.foreign.control_endpoints.fields.endpoint_type_id;angular.forEach(a.endpointTypesSelected,
function(a){f==a&&(c=!0)})}c||d?a.controlList[b.id]=b:delete a.controlList[b.id]});return a.controlList};this.endpointList={};this.getEndpoints=function(){angular.forEach(a.endpoints.indexed,function(b){var c=!1,d=!0;if(angular.isArray(a.endpointTypesSelected)&&0<a.endpointTypesSelected.length){var d=!1,h=b.fields.endpoint_type_id;angular.forEach(a.endpointTypesSelected,function(a){h==a&&(c=!0)})}c||d?a.endpointList[b.id]=b:delete a.endpointList[b.id]});return a.endpointList};this.clearEndpointTypes=
