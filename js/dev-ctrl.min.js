...
this.config=c.config;var e={};angular.isObject(this.config.rooms)||(this.config.rooms={});angular.isObject(this.config.rooms[d.id])||(this.config.rooms[this.id]={groups:{}});e=this.config.rooms[this.id];this.getGroups=function(){var a={};angular.forEach(e.groups,function(e,b){a[b]=!0});angular.forEach(d.panels,function(b){angular.isDefined(e.groups[b.fields.grouping])||(e.groups[b.fields.grouping]={opened:!1});a[b.fields.grouping]=!1});angular.forEach(a,function(a,b){a&&delete e.groups[b]});return e.groups};
this.getRoomEndpoints=function(a){var b={},e=!angular.isDefined(a);angular.forEach(d.obj.referenced.panels,function(c,d){(e||c.fields.grouping==a)&&angular.forEach(c.referenced.panel_controls,function(a,e){var c=a.foreign.controls.foreign.control_endpoints;angular.isDefined(b[c.id])||(b[c.id]=c)})});return b};this.panelControls=function(a){if(angular.isDefined(a.referenced.panel_controls))return a.referenced.panel_controls};this.toggleGroup=function(a){a.opened=!a.opened;c.updateConfig()}}];DevCtrl.MainCtrl=["$state","$mdMedia","DataService","MenuService",function(b,c,a,d){this.msg="Hello World!";this.tiles=[{img:"/images/orc.png"},{img:"/images/pict.png"},{img:"/images/sage.png"}];this.$state=b;this.schema=a.schema;this.menu=d;this.$mdMedia=c;this.control_endpoints=a.getTable("control_endpoints");this.config=a.config;this.user=a.dataModel.user;this.updateConfig=function(){a.updateConfig()};this.go=function(a){angular.isString(a)?b.go(a):b.go(a.name,a.params)};this.addEndpoint=function(b){a.editRecord(b,
...
DevCtrl.DataService.factory=["$window","$http","$mdToast","$timeout","$q","socketFactory","$mdDialog","$location",function(b,c,a,d,e,f,k,l){var h={user:{username:null,admin:!1,client_id:0},applog:[]},n={},p=!1,q=c.get("schema.php").success(function(a){angular.isDefined(a.schema)&&angular.forEach(a.schema,function(a,b){var t=g.getSchema(b);angular.merge(t,a);angular.isDefined(t.foreign_keys)&&angular.forEach(t.foreign_keys,function(a,b){t.referenced[a]=b})});p=!0}).error(function(a){g.errorToast(a)}),r=
io("https://devctrl.dwi.ufl.edu/"),u=f({ioSocket:r}),v={},m={};f={editEnabled:!0};"undefined"!==typeof b.localStorage&&(r=b.localStorage.config,angular.isString(r)?f=JSON.parse(r):b.localStorage.config=JSON.stringify(f));var g={config:f,messenger:u,dataModel:h,schema:n,addRow:function(a,b){c.post("data.php",a).then(function(c){var e=Object.keys(c.data.add[a.tableName])[0];console.log("new record "+e+"added to "+a.tableName);g.loadData(c.data);c=h[a.tableName].indexed[e];angular.forEach(a,function(b,
c){"table"!=c&&(a[c]=null)});angular.isFunction(b)&&b(c)},function(a){g.errorToast(a.data)})},dialogClose:function(){k.hide()},deleteRow:function(a){var b="data.php/"+a.tableName+"/"+a.id,e=!1;angular.forEach(a.referenced,function(a,b){0<Object.keys(a).length&&(e=b)});e?g.errorToast({error:"Cannot delete "+a.tableName+" record due to foreign key constraint on "+e}):c.delete(b).success(function(a){g.loadData(a)}).error(function(a){g.errorToast(a)})},editEnum:function(a,b,c,e){angular.isObject(e)||
(e={});k.show({targetEvent:a,locals:{myEnum:b,enumRefRecord:c,options:e},controller:DevCtrl.EnumEditor.Ctrl,controllerAs:"editor",bindToController:!0,templateUrl:"ng/enum-editor.html",clickOutsideToClose:!0,hasBackdrop:!1})},editRecord:function(a,b,c,e){"0"!==b?b=g.getRowRef(c,b):(b=g.getNewRowRef(c),angular.isObject(e)&&angular.merge(b.fields,e));k.show({targetEvent:a,locals:{obj:b},controller:DevCtrl.Record.Ctrl,controllerAs:"record",bindToController:!0,templateUrl:"ng/record.html",clickOutsideToClose:!0,
hasBackdrop:!1})},editRecordClose:function(){k.hide()},errorToast:function(b){var c="An unknown error has occured";angular.isDefined(b.error)&&(c=b.error);a.show({templateUrl:"ng/error-toast.html",locals:{message:c},controllerAs:"toast",controller:"RoomsCtrl",bindToController:!0,position:"top right",hideDelay:3E3})},getAdminAuth:function(a){var b="admin_auth.php";a&&(b="admin_auth.php?logon=1&location="+l.path());return c.get(b).then(function(a){angular.isDefined(a.data.user)?angular.merge(h.user,
a.data.user):console.log("admin_auth did not return an admin status")},function(b){"401"==b.status&&(a&&angular.isDefined(b.data.location)?window.location=b.data.location:(angular.isDefined(b.data.user)&&angular.merge(h.user,b.data.user),g.errorToast(b.data)))})},getLog:function(){return c.get("log.php").then(function(a){angular.isDefined(a.data.applog)&&(h.applog.length=0,angular.merge(h.applog,a.data.applog))})},getMData:function(a,b){var c={table:a,query:b};g.getMProm=e(function(a,b){u.emit("get-data",
c,function(b){console.log("data received:"+b);g.loadData(b);a(!0)})});return g.getMProm},getNewRowRef:function(a){var b={id:"0",referenced:{},tableName:a,fields:{}};a=g.getSchema(a);angular.forEach(a.fields,function(a,c){b.fields[c]=""});return b},getRowRef:function(a,b){if(!angular.isDefined(a))return console.error("error looking up record for undefined table"),{};if(!angular.isDefined(b)||null===b)return console.error("error looking up %s record for undefined key",a),{};var c=this.getTableRef(a);
...
angular.isDefined(n[a])||(n[a]={referenced:{},foreign_keys:{}});return n[a]},getSchemaPromise:function(){if(!p)return q},getTable:function(a){angular.isDefined(h[a])?angular.isDefined(m[a])||(m[a]=this.getTablePromise(a)):(this.getTableRef(a),m[a]=this.getTablePromise(a));return h[a]},getTablePromise:function(a){if(angular.isDefined(m[a]))return m[a];if(angular.isDefined(a))return m[a]=c.get("data.php?table="+a).then(function(b){g.loadData(b.data);return p?h[a]:q.then(function(){return h[a]})},function(a){g.errorToast(a.data)}),
m[a];console.error("error: attempt to fetch undefined table!")},getTableRef:function(a){angular.isDefined(h[a])||(h[a]={listed:[],indexed:{},loaded:!1});return h[a]},isAdminAuthorized:function(){return h.user.admin},loadData:function(a){if(p)g.loadDataKernel(a);else return q=q.then(function(){g.loadDataKernel(a)})},loadDataKernel:function(a){angular.isDefined(a.update)&&(angular.isDefined(a.add)?angular.merge(a.add,a.update):a.add=a.update);angular.isDefined(a.add)&&angular.forEach(a.add,function(a,
b){var c=g.getSchema(b).foreign_keys;angular.forEach(a,function(a,e){var d=g.getRowRef(b,e);angular.merge(d.fields,a);d.loaded=!0;angular.forEach(c,function(a,c){if(angular.isDefined(d.fields[c])&&null!==d.fields[c]){var e=g.getRowRef(a,d.fields[c]);angular.isDefined(e.referenced[b])||(e.referenced[b]={});e.referenced[b][d.id]=d;d.foreign[a]=e;d.foreign[c]=e}else d.foreign[a]=null,d.foreign[c]=null})})});angular.isDefined(a.delete)&&angular.forEach(a.delete,function(a,b){var c=g.getSchema(b).pk;angular.forEach(a,
function(a,c){angular.forEach(h[b].indexed[c].foreign,function(a,e){angular.isDefined(a.referenced[b][c])&&delete a.referenced[b][c]});delete h[b].indexed[c]});h[b].listed.length=0;angular.forEach(h[b].indexed,function(a,e){angular.isString(c)?h[b].listed.push(a):angular.forEach(a,function(a){h[b].listed.push(a)})})})},revokeAdminAuth:function(){c.get("admin_auth_revoke.php").then(function(a){angular.isDefined(a.data.user)?angular.merge(h.user,a.data.user):console.log("revoke admin_auth did not return user info")},
function(a){g.errorToast(a.data)})},showControlLog:function(a,b){g.getMData("control_log",{control_id:b.id}).then(function(){k.show({targetEvent:a,locals:{ctrl:b},controller:DevCtrl.CtrlLog.Ctrl,controllerAs:"ctrlLog",bindToController:!0,templateUrl:"ng/ctrl-log.html",clickOutsideToClose:!0,hasBackdrop:!1})})},updateConfig:function(){"undefined"!==typeof b.localStorage&&(b.localStorage.config=JSON.stringify(g.config))},updateControlValue:function(a){angular.isDefined(v[a.id])&&d.cancel(v[a.id]);v[a.id]=
d(function(a,b){var e="control.php/"+a.id;pendingDebounce=!1;c.put(e,a.fields).success(function(a){b.loadData(a)}).error(function(a){b.errorToast(a)})},200,!0,a,g)},updateRow:function(a){c.put("data.php/"+a.tableName+"/"+a.id,a.fields).success(function(a){g.loadData(a)}).error(function(a){g.errorToast(a)})}};u.on("control-data",function(a){g.loadData(a)});u.on("log-data",function(a){g.dataModel.applog.push(a)});return g}];DevCtrl.Slider2d={};
...
a.config;this.type=this.template.fields.usertype;this.enums=a.getTable("enums");this.enumVals=a.getTable("enum_vals");var c=this;this.normalizedValue=function(){var a=c.ctrl.fields.value,b=c.intConfig("max"),d=c.intConfig("min"),a=a<d?d:a;return((a>b?b:a)+(0-d))*(b-d)/100};this.updateValue=function(b){a.updateControlValue(c.ctrl)};this.editOptions=function(b){a.editEnum(b,null,c.ctrl,{title:"Edit "+c.name+" options"})};this.editTemplateOptions=function(b){a.editEnum(b,null,c.template,{title:"Edit "+
c.ctrlName()+" options"})};this.selectMenuItem=function(a){c.ctrl.fields.value=a;c.updateValue()};this.selectOptions=function(){var a=c.ctrl.fields.enum_id,b={};0<a?b=c.enums.indexed[a].referenced.enum_vals:0<c.template.fields.enum_id&&(b=c.template.foreign.enums.referenced.enum_vals);return b};this.selectValueName=function(){var a=c.selectOptions(),b=c.ctrl.fields.value,d="";angular.forEach(a,function(a){a.fields.value==b&&(d=a.fields.name)});return d};this.showLog=function(b){a.showControlLog(b,
c.ctrl)};this.editPanelControl=function(b){a.editRecord(b,c.panelControl.id,"panel_controls")};this.editControl=function(b){a.editRecord(b,c.ctrl.id,"controls")};this.editTemplate=function(b){a.editRecord(b,c.template.id,"control_templates")}},controllerAs:"ctrl",templateUrl:"ng/ctrl.html"}}];DevCtrl.Record={};
...
this.addTemplate=function(a){c.editRecord(a,"0","control_templates",{endpoint_type_id:d.obj.fields.endpoint_type_id})};this.editEndpoint=function(a){c.editRecord(a,this.endpointId,"control_endpoints")}}];DevCtrl.PanelControlSelector={};
...
!1,a.endpointSelected==b.foreign.control_endpoint_id.id&&(c=!0);else if(angular.isArray(a.endpointTypesSelected)&&0<a.endpointTypesSelected.length){var f=!1,l=b.foreign.control_endpoints.fields.endpoint_type_id;angular.forEach(a.endpointTypesSelected,function(a){l==a&&(c=!0)})}c||f?a.controlList[b.id]=b:delete a.controlList[b.id]});return a.controlList};this.endpointList={};this.getEndpoints=function(){angular.forEach(a.endpoints.indexed,function(b){var c=!1,f=!0;if(angular.isArray(a.endpointTypesSelected)&&
...
DevCtrl.Table.Ctrl).controller("RecordCtrl",DevCtrl.Record.Ctrl).controller("RoomCtrl",DevCtrl.Room.Ctrl).controller("RoomsCtrl",DevCtrl.Rooms.Ctrl).config(DevCtrl.stateConfig).run(["$rootScope",function(b){b.$on("$stateChangeStart",function(b,a,d,e,f){console.log("$stateChangeStart to "+a.to+"- fired when the transition begins. toState,toParams : \n",a,d)});b.$on("$stateChangeError",function(b,a,d,e,f){console.log("$stateChangeError - fired when an error occurs during transition.");console.log(arguments)});
b.$on("$stateChangeSuccess",function(b,a,d,e,f){console.log("$stateChangeSuccess to "+a.name+"- fired once the state transition is complete.")});b.$on("$viewContentLoaded",function(b){console.log("$viewContentLoaded - fired after dom rendered",b)});b.$on("$stateNotFound",function(b,a,d,e){console.log("$stateNotFound "+a.to+"  - fired when a state cannot be found by its name.");console.log(a,d,e)})}]);
