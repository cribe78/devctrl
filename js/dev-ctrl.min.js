...
this.addEndpoint=function(b){a.editRecord(b,"0","control_endpoints")};this.addEndpointType=function(b){a.editRecord(b,"0","endpoint_types")};this.dataModel=a.dataModel;this.title="DevCtrl";this.top=!0}];DevCtrl.Toolbar={};
DevCtrl.Toolbar.Directive=["$mdMedia","$state","MenuService","DataService",function(d,c,a,b){return{scope:!0,bindToController:{title:"="},controller:function(a,b,c){var e=this;this.menu=b;this.user=c.dataModel.user;this.config=c.config;this.$mdMedia=d;this.pageTitle=function(){return angular.isDefined(e.title)?e.title:a.current.title||a.params.name};this.adminLogin=function(){c.getAdminAuth(!0)};this.revokeAdmin=function(){c.revokeAdminAuth()};this.toggleSidenav=function(a){$mdSidenav(a).toggle()};this.updateConfig=
function(){c.updateConfig()}},transclude:!0,controllerAs:"toolbar",templateUrl:"ng/toolbar.html"}}];DevCtrl.AdminOnly={};DevCtrl.AdminOnly.Directive=["$compile","DataService",function(d,c){return{restrict:"A",replace:!1,terminal:!0,priority:1E3,link:function(a,b,h){var g=!1;"invert"==b.attr("devctrl-admin-only")&&(g=!0);b.removeAttr("devctrl-admin-only");b.attr("ng-if","adminEnabled()");a.adminEnabled=function(){var a=c.isAdminAuthorized()&&c.config.editEnabled;g&&(a=!a);return a};d(b)(a)}}}];DevCtrl.Menu={};DevCtrl.Menu.Directive=["MenuService","$state",function(d,c){return{scope:!0,bindToController:{},controller:function(a,b){this.service=a},controllerAs:"menu",templateUrl:"ng/menu.html"}}];DevCtrl.DataService={};
DevCtrl.DataService.factory=["$window","$http","$mdToast","$timeout","socketFactory","$mdDialog","$location",function(d,c,a,b,h,g,k){var e={user:{username:null,admin:!1},applog:[]},l={},q=!1,r=c.get("schema.php").success(function(a){angular.isDefined(a.schema)&&angular.forEach(a.schema,function(a,b){var c=f.getSchema(b);angular.merge(c,a);angular.isDefined(c.foreign_keys)&&angular.forEach(c.foreign_keys,function(a,b){c.referenced[a]=b})});q=!0}).error(function(a){f.errorToast(a)}),m=io("https://devctrl.dwi.ufl.edu/");
h=h({ioSocket:m});var p={},n={},m={editEnabled:!0};if("undefined"!==typeof d.localStorage){var t=d.localStorage.config;angular.isString(t)?m=JSON.parse(t):d.localStorage.config=JSON.stringify(m)}var f={config:m,messenger:h,dataModel:e,schema:l,addRow:function(a,b){c.post("data.php",a).then(function(c){var d=Object.keys(c.data.add[a.tableName])[0];console.log("new record "+d+"added to "+a.tableName);f.loadData(c.data);c=e[a.tableName].indexed[d];angular.forEach(a,function(b,c){"table"!=c&&(a[c]=null)});
angular.isFunction(b)&&b(c)},function(a){f.errorToast(a.data)})},deleteRow:function(a){var b="data.php/"+a.tableName+"/"+a.id,d=!1;angular.forEach(a.referenced,function(a,b){0<Object.keys(a).length&&(d=b)});d?f.errorToast({error:"Cannot delete "+a.tableName+" record due to foreign key constraint on "+d}):c.delete(b).success(function(a){f.loadData(a)}).error(function(a){f.errorToast(a)})},editEnum:function(a,b,c,d){angular.isObject(d)||(d={});g.show({targetEvent:a,locals:{myEnum:b,enumRefRecord:c,
options:d},controller:DevCtrl.EnumEditor.Ctrl,controllerAs:"editor",bindToController:!0,templateUrl:"ng/enum-editor.html",clickOutsideToClose:!0,hasBackdrop:!1})},editRecord:function(a,b,c,d){"0"!==b?b=f.getRowRef(c,b):(b=f.getNewRowRef(c),angular.isObject(d)&&angular.merge(b.fields,d));g.show({targetEvent:a,locals:{obj:b},controller:DevCtrl.Record.Ctrl,controllerAs:"record",bindToController:!0,templateUrl:"ng/record.html",clickOutsideToClose:!0,hasBackdrop:!1})},editRecordClose:function(){g.hide()},
...
function(b){"401"==b.status&&(a&&angular.isDefined(b.data.location)?window.location=b.data.location:(angular.isDefined(b.data.user)&&angular.merge(e.user,b.data.user),f.errorToast(b.data)))})},getLog:function(){return c.get("log.php").then(function(a){angular.isDefined(a.data.applog)&&(e.applog.length=0,angular.merge(e.applog,a.data.applog))})},getNewRowRef:function(a){var b={id:"0",referenced:{},tableName:a,fields:{}};a=f.getSchema(a);angular.forEach(a.fields,function(a,c){b.fields[c]=""});return b},
...
a)},getMenu:function(){angular.isDefined(e.menu)||(e.menu={items:{}},c.get("menu.php").success(function(a){angular.isDefined(a.menu.items)?(console.log("test/menu.php loaded"),e.menu.items=a.menu.items):console.log("test/menu.php did not return a valid menu object")}).error(function(a){f.errorToast(a)}));return e.menu},getSchema:function(a){l.loaded||angular.isDefined(l[a])||(l[a]={referenced:{},foreign_keys:{}});return l[a]},getTable:function(a){angular.isDefined(e[a])?angular.isDefined(n[a])||(n[a]=
this.getTablePromise(a)):(this.getTableRef(a),n[a]=this.getTablePromise(a));return e[a]},getTablePromise:function(a){if(angular.isDefined(n[a]))return n[a];if(angular.isDefined(a))return c.get("data.php?table="+a).success(function(a){f.loadData(a)}).error(function(a){f.errorToast(a)});console.error("error: attempt to fetch undefined table!")},getTableRef:function(a){angular.isDefined(e[a])||(e[a]={listed:[],indexed:{},loaded:!1});return e[a]},isAdminAuthorized:function(){return e.user.admin},loadData:function(a){q?
f.loadDataKernel(a):r=r.then(function(){f.loadDataKernel(a)})},loadDataKernel:function(a){angular.isDefined(a.update)&&angular.forEach(a.update,function(a,b){angular.forEach(a,function(a,c){var d=f.getRowRef(b,c);angular.merge(d.fields,a)})});angular.isDefined(a.add)&&angular.forEach(a.add,function(a,b){var c=f.getSchema(b),d=c.pk,h=c.foreign_keys;angular.forEach(a,function(a,c){if(angular.isString(d)){var g=f.getRowRef(b,c);angular.merge(g.fields,a);g.loaded=!0;angular.forEach(h,function(a,c){if(null!==
g.fields[c]){var d=f.getRowRef(a,g.fields[c]);angular.isDefined(d.referenced[b])||(d.referenced[b]={});d.referenced[b][g.id]=g;g.foreign[a]=d;g.foreign[c]=d}else g.foreign[a]=null,g.foreign[c]=null})}else console.error("Error loading %s, multi-keyed records not supported",b)})});angular.isDefined(a.delete)&&angular.forEach(a.delete,function(a,b){var c=f.getSchema(b).pk;angular.forEach(a,function(a,c){angular.forEach(e[b].indexed[c].foreign,function(a,d){angular.isDefined(a.referenced[b][c])&&delete a.referenced[b][c]});
delete e[b].indexed[c]});e[b].listed.length=0;angular.forEach(e[b].indexed,function(a,d){angular.isString(c)?e[b].listed.push(a):angular.forEach(a,function(a){e[b].listed.push(a)})})})},revokeAdminAuth:function(){c.get("admin_auth_revoke.php").then(function(a){angular.isDefined(a.data.user)?angular.merge(e.user,a.data.user):console.log("revoke admin_auth did not return user info")},function(a){f.errorToast(a.data)})},updateConfig:function(){"undefined"!==typeof d.localStorage&&(d.localStorage.config=
JSON.stringify(f.config))},updateControlValue:function(a){angular.isDefined(p[a.id])&&b.cancel(p[a.id]);p[a.id]=b(function(a,b){var d="control.php/"+a.id;pendingDebounce=!1;c.put(d,a.fields).success(function(a){b.loadData(a)}).error(function(a){b.errorToast(a)})},200,!0,a,f)},updateRow:function(a){c.put("data.php/"+a.tableName+"/"+a.id,a.fields).success(function(a){f.loadData(a)}).error(function(a){f.errorToast(a)})}};h.on("control-data",function(a){f.loadData(a)});h.on("log-data",function(a){f.dataModel.applog.push(a)});
...
c[1]:0;d=parseInt(d)};this.setXYVals();this.updateValue=function(){c.updateControlValue(a.control.ctrl)}},controllerAs:"slider2d",templateUrl:"ng/controls/slider2d-directive.html"}}];DevCtrl.EnumSelect={};
...
DevCtrl.MenuService.factory=["$state","DataService",function(d,c){var a={go:function(a){angular.isString(a)?d.go(a):d.go(a.name,a.params)},pageTitle:function(){return d.current.title||d.params.name},parentState:function(){return d.get("^")},isFirstLevel:function(){return""===d.current.name||""===d.get("^").name},items:{},states:function(){return d.get()},menuItems:function(){var b=d.get();angular.forEach(b,function(b,c){""!=b.name&&(b.isOpened=d.includes(b),""==d.get("^",b).name&&(a.items[b.name]=
b,angular.isDefined(b.substates)||(b.substates={})),angular.isDefined(b.data.title)&&(b.title=b.data.title))});angular.forEach(b,function(b,g){if(""!=b.name){var k=d.get("^",b);if(angular.isDefined(a.items[k.name]))if(angular.isDefined(b.data.listByName)){var e=c.getTable(b.data.listByName).listed;angular.forEach(e,function(a){angular.isDefined(k.substates[a.id])?(k.substates[a.id].params.name=a.fields.name,k.substates[a.id].title=a.fields.name):k.substates[a.id]={name:b.name,params:{name:a.fields.name,
id:a.id},title:a.fields.name}})}else a.items[k.name].substates[b.name]=b}});return a.items}};return a}];DevCtrl.EndpointStatus={};
...
!1,a.endpointSelected==b.foreign.control_endpoint_id.id&&(c=!0);else if(angular.isArray(a.endpointTypesSelected)&&0<a.endpointTypesSelected.length){var d=!1,e=b.foreign.control_endpoints.fields.endpoint_type_id;angular.forEach(a.endpointTypesSelected,function(a){e==a&&(c=!0)})}c||d?a.controlList[b.id]=b:delete a.controlList[b.id]});return a.controlList};this.endpointList={};this.getEndpoints=function(){angular.forEach(a.endpoints.indexed,function(b){var c=!1,d=!0;if(angular.isArray(a.endpointTypesSelected)&&
...
DevCtrl.Record.Ctrl).controller("RoomCtrl",DevCtrl.Room.Ctrl).controller("RoomsCtrl",DevCtrl.Rooms.Ctrl).config(DevCtrl.stateConfig).run(["$rootScope",function(d){d.$on("$stateChangeStart",function(c,a,b,d,g){console.log("$stateChangeStart to "+a.to+"- fired when the transition begins. toState,toParams : \n",a,b)});d.$on("$stateChangeError",function(c,a,b,d,g){console.log("$stateChangeError - fired when an error occurs during transition.");console.log(arguments)});d.$on("$stateChangeSuccess",function(c,
a,b,d,g){console.log("$stateChangeSuccess to "+a.name+"- fired once the state transition is complete.")});d.$on("$viewContentLoaded",function(c){console.log("$viewContentLoaded - fired after dom rendered",c)});d.$on("$stateNotFound",function(c,a,b,d){console.log("$stateNotFound "+a.to+"  - fired when a state cannot be found by its name.");console.log(a,b,d)})}]);
