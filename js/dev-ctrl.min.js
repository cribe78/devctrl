...
b=this.config.rooms[this.id];this.getGroups=function(){var e={};angular.forEach(b.groups,function(a,b){e[b]=!0});angular.forEach(a.panels,function(a){angular.isDefined(b.groups[a.fields.grouping])||(b.groups[a.fields.grouping]={opened:!1});e[a.fields.grouping]=!1});angular.forEach(e,function(a,e){a&&delete b.groups[e]});return b.groups};this.panelControls=function(a){if(angular.isDefined(a.referenced.panel_controls))return a.referenced.panel_controls};this.toggleGroup=function(a){a.opened=!a.opened;
...
this.addEndpoint=function(b){a.editRecord(b,"0","control_endpoints")};this.addEndpointType=function(b){a.editRecord(b,"0","endpoint_types")};this.dataModel=a.dataModel;this.title="DevCtrl";this.top=!0;this.adminEnabled=function(){return a.isAdminAuthorized()};this.adminLogin=function(){a.getAdminAuth(!0)};this.revokeAdmin=function(){a.revokeAdminAuth()}}];DevCtrl.AdminOnly={};DevCtrl.AdminOnly.Directive=["$compile","DataService",function(d,c){return{restrict:"A",replace:!1,terminal:!0,priority:1E3,link:function(a,b,e){var h=!1;"invert"==b.attr("devctrl-admin-only")&&(h=!0);b.removeAttr("devctrl-admin-only");b.attr("ng-if","adminEnabled()");a.adminEnabled=function(){var a=c.isAdminAuthorized()&&c.config.editEnabled;h&&(a=!a);return a};d(b)(a)}}}];DevCtrl.Menu={};DevCtrl.Menu.Directive=["MenuService","$state",function(d,c){return{scope:!0,bindToController:{},controller:function(a,b){this.service=a},controllerAs:"menu",templateUrl:"ng/menu.html"}}];DevCtrl.DataService={};
DevCtrl.DataService.factory=["$window","$http","$mdToast","$timeout","socketFactory","$mdDialog","$location",function(d,c,a,b,e,h,k){var f={user:{username:null,admin:!1},applog:[]},l={},r=!1,t=c.get("schema.php").success(function(a){angular.isDefined(a.schema)&&angular.forEach(a.schema,function(a,b){var p=g.getSchema(b);angular.merge(p,a);angular.isDefined(p.foreign_keys)&&angular.forEach(p.foreign_keys,function(a,b){p.referenced[a]=b})});r=!0}).error(function(a){g.errorToast(a)}),m=io("https://devctrl.dwi.ufl.edu/");
e=e({ioSocket:m});var q={},n={},m={editEnabled:!0};if("undefined"!==typeof d.localStorage){var u=d.localStorage.config;angular.isString(u)?m=JSON.parse(u):d.localStorage.config=JSON.stringify(m)}var g={config:m,messenger:e,dataModel:f,schema:l,addRow:function(a,b){c.post("data.php",a).then(function(c){var e=Object.keys(c.data.add[a.tableName])[0];console.log("new record "+e+"added to "+a.tableName);g.loadData(c.data);c=f[a.tableName].indexed[e];angular.forEach(a,function(b,c){"table"!=c&&(a[c]=null)});
angular.isFunction(b)&&b(c)},function(a){g.errorToast(a.data)})},deleteRow:function(a){var b="data.php/"+a.tableName+"/"+a.id,e=!1;angular.forEach(a.referenced,function(a,b){0<Object.keys(a).length&&(e=b)});e?g.errorToast({error:"Cannot delete "+a.tableName+" record due to foreign key constraint on "+e}):c.delete(b).success(function(a){g.loadData(a)}).error(function(a){g.errorToast(a)})},editEnum:function(a,b,c,e){angular.isObject(e)||(e={});h.show({targetEvent:a,locals:{myEnum:b,enumRefRecord:c,
options:e},controller:DevCtrl.EnumEditor.Ctrl,controllerAs:"editor",bindToController:!0,templateUrl:"ng/enum-editor.html",clickOutsideToClose:!0,hasBackdrop:!1})},editRecord:function(a,b,c,e){"0"!==b?b=g.getRowRef(c,b):(b=g.getNewRowRef(c),angular.isObject(e)&&angular.merge(b.fields,e));h.show({targetEvent:a,locals:{obj:b},controller:DevCtrl.Record.Ctrl,controllerAs:"record",bindToController:!0,templateUrl:"ng/record.html",clickOutsideToClose:!0,hasBackdrop:!1})},editRecordClose:function(){h.hide()},
...
function(b){"401"==b.status&&(a&&angular.isDefined(b.data.location)?window.location=b.data.location:(angular.isDefined(b.data.user)&&angular.merge(f.user,b.data.user),g.errorToast(b.data)))})},getLog:function(){return c.get("log.php").then(function(a){angular.isDefined(a.data.applog)&&(f.applog.length=0,angular.merge(f.applog,a.data.applog))})},getNewRowRef:function(a){var b={id:"0",referenced:{},tableName:a,fields:{}};a=g.getSchema(a);angular.forEach(a.fields,function(a,c){b.fields[c]=""});return b},
getRowRef:function(a,b){if(!angular.isDefined(a))return console.error("error looking up record for undefined table"),{};if(!angular.isDefined(b)||null===b)return console.error("error looking up %s record for undefined key",a),{};var c=this.getTableRef(a);if(angular.isString(l[a].pk))return angular.isDefined(c.indexed[b])||(c.indexed[b]={fields:{},foreign:{},id:b,loaded:!1,referenced:{},tableName:a},c.listed.push(c.indexed[b])),c.indexed[b];console.error("multi column keys not supported, table %s",
a)},getMenu:function(){angular.isDefined(f.menu)||(f.menu={items:{}},c.get("menu.php").success(function(a){angular.isDefined(a.menu.items)?(console.log("test/menu.php loaded"),f.menu.items=a.menu.items):console.log("test/menu.php did not return a valid menu object")}).error(function(a){g.errorToast(a)}));return f.menu},getSchema:function(a){l.loaded||angular.isDefined(l[a])||(l[a]={referenced:{},foreign_keys:{}});return l[a]},getTable:function(a){angular.isDefined(f[a])?angular.isDefined(n[a])||(n[a]=
this.getTablePromise(a)):(this.getTableRef(a),n[a]=this.getTablePromise(a));return f[a]},getTablePromise:function(a){if(angular.isDefined(n[a]))return n[a];if(angular.isDefined(a))return c.get("data.php?table="+a).success(function(a){g.loadData(a)}).error(function(a){g.errorToast(a)});console.error("error: attempt to fetch undefined table!")},getTableRef:function(a){angular.isDefined(f[a])||(f[a]={listed:[],indexed:{},loaded:!1});return f[a]},isAdminAuthorized:function(){return f.user.admin},loadData:function(a){r?
g.loadDataKernel(a):t=t.then(function(){g.loadDataKernel(a)})},loadDataKernel:function(a){angular.isDefined(a.update)&&angular.forEach(a.update,function(a,b){angular.forEach(a,function(a,c){var e=g.getRowRef(b,c);angular.merge(e.fields,a)})});angular.isDefined(a.add)&&angular.forEach(a.add,function(a,b){var c=g.getSchema(b),e=c.pk,d=c.foreign_keys;angular.forEach(a,function(a,c){if(angular.isString(e)){var h=g.getRowRef(b,c);angular.merge(h.fields,a);h.loaded=!0;angular.forEach(d,function(a,c){if(null!==
h.fields[c]){var e=g.getRowRef(a,h.fields[c]);angular.isDefined(e.referenced[b])||(e.referenced[b]={});e.referenced[b][h.id]=h;h.foreign[a]=e;h.foreign[c]=e}else h.foreign[a]=null,h.foreign[c]=null})}else console.error("Error loading %s, multi-keyed records not supported",b)})});angular.isDefined(a.delete)&&angular.forEach(a.delete,function(a,b){var c=g.getSchema(b).pk;angular.forEach(a,function(a,c){angular.forEach(f[b].indexed[c].foreign,function(a,e){angular.isDefined(a.referenced[b][c])&&delete a.referenced[b][c]});
delete f[b].indexed[c]});f[b].listed.length=0;angular.forEach(f[b].indexed,function(a,e){angular.isString(c)?f[b].listed.push(a):angular.forEach(a,function(a){f[b].listed.push(a)})})})},revokeAdminAuth:function(){c.get("admin_auth_revoke.php").then(function(a){angular.isDefined(a.data.user)?angular.merge(f.user,a.data.user):console.log("revoke admin_auth did not return user info")},function(a){g.errorToast(a.data)})},updateConfig:function(){"undefined"!==typeof d.localStorage&&(d.localStorage.config=
JSON.stringify(g.config))},updateControlValue:function(a){angular.isDefined(q[a.id])&&b.cancel(q[a.id]);q[a.id]=b(function(a,b){var e="control.php/"+a.id;pendingDebounce=!1;c.put(e,a.fields).success(function(a){b.loadData(a)}).error(function(a){b.errorToast(a)})},200,!0,a,g)},updateRow:function(a){c.put("data.php/"+a.tableName+"/"+a.id,a.fields).success(function(a){g.loadData(a)}).error(function(a){g.errorToast(a)})}};e.on("control-data",function(a){g.loadData(a)});e.on("log-data",function(a){g.dataModel.applog.push(a)});
return g}];DevCtrl.Slider2d={};
DevCtrl.Slider2d.Directive=["DataService",function(d){return{scope:{control:"="},bindToController:!0,controller:function(c){var a=this,b,e;this.xValue=function(c){angular.isDefined(c)?(b=c,a.control.ctrl.fields.value=b+","+e):this.setXYVals();return b};this.yValue=function(c){angular.isDefined(c)?(e=c,a.control.ctrl.fields.value=b+","+e):this.setXYVals();return e};this.setXYVals=function(){var c=a.control.ctrl.fields.value.split(",");b=angular.isDefined(c[0])?c[0]:0;b=parseInt(b);e=angular.isDefined(c[1])?
c[1]:0;e=parseInt(e)};this.setXYVals();this.updateValue=function(){c.updateControlValue(a.control.ctrl)}},controllerAs:"slider2d",templateUrl:"ng/controls/slider2d-directive.html"}}];DevCtrl.EnumSelect={};
...
this.template.fields.usertype;this.enums=c.getTable("enums");this.enumVals=c.getTable("enum_vals");var a=this;this.normalizedValue=function(){var b=a.ctrl.fields.value,c=a.intConfig("max"),d=a.intConfig("min"),b=b<d?d:b;return((b>c?c:b)+(0-d))*(c-d)/100};this.updateValue=function(b){c.updateControlValue(a.ctrl)};this.editOptions=function(b){c.editEnum(b,null,a.ctrl,{title:"Edit "+a.name+" options"})};this.editTemplateOptions=function(b){c.editEnum(b,null,a.template,{title:"Edit "+a.ctrlName()+" options"})};
this.selectMenuItem=function(b){a.ctrl.fields.value=b;a.updateValue()};this.selectOptions=function(){var b=a.ctrl.fields.enum_id,c={};0<b?c=a.enums.indexed[b].referenced.enum_vals:0<a.template.fields.enum_id&&(c=a.template.foreign.enums.referenced.enum_vals);return c};this.selectValueName=function(){var b=a.selectOptions(),c=a.ctrl.fields.value,d="";angular.forEach(b,function(a){a.fields.value==c&&(d=a.fields.name)});return d};this.editPanelControl=function(b){c.editRecord(b,a.panelControl.id,"panel_controls")};
this.editControl=function(b){c.editRecord(b,a.ctrl.id,"controls")};this.editTemplate=function(b){c.editRecord(b,a.template.id,"control_templates")}},controllerAs:"ctrl",templateUrl:"ng/ctrl.html"}}];DevCtrl.Record={};
DevCtrl.Record.Ctrl=["DataService",function(d){this.newRow="0"===this.obj.id;this.schema=d.getSchema(this.obj.tableName);var c=this;this.addRow=function(){d.addRow(c.obj);d.editRecordClose()};this.deleteRow=function(){d.deleteRow(c.obj);d.editRecordClose()};this.updateRow=function(){d.updateRow(c.obj);d.editRecordClose()};this.cloneRow=function(){var a=d.getNewRowRef(c.obj.tableName);a.fields=c.obj.fields;d.addRow(a,function(a){c.obj=a;angular.isDefined(c.obj.fields.name)&&(c.obj.fields.name="")})};
this.close=function(){d.editRecordClose()}}];DevCtrl.Log={};DevCtrl.Log.Ctrl=["DataService",function(d){this.applog=d.dataModel.applog}];DevCtrl.Log.Resolve={loadLog:["DataService",function(d){return d.getLog()}]};DevCtrl.MenuService={};
...
(b.title=b.data.title))});angular.forEach(b,function(b,h){if(""!=b.name){var k=d.get("^",b);if(angular.isDefined(a.items[k.name]))if(angular.isDefined(b.data.listByName)){var f=c.getTable(b.data.listByName).listed;angular.forEach(f,function(a){angular.isDefined(k.substates[a.id])?(k.substates[a.id].params.name=a.fields.name,k.substates[a.id].title=a.fields.name):k.substates[a.id]={name:b.name,params:{name:a.fields.name,id:a.id},title:a.fields.name}})}else a.items[k.name].substates[b.name]=b}});return a.items}};
...
DevCtrl.PanelControlSelector.Ctrl=["$mdDialog","DataService",function(d,c){var a=this;this.endpointTypes=c.getTable("endpoint_types");this.endpoints=c.getTable("control_endpoints");this.controls=c.getTable("controls");this.control_templates=c.getTable("control_templates");this.newPanelControl=c.getNewRowRef("panel_controls");this.newPanelControl.fields.panel_id=this.panelId;this.endpointTypesSelected=[];this.endpointsSelected=[];this.endpointsSelected="0";this.getControlName=function(a){var c=a.fields.name;
""==a.fields.name&&(c=a.foreign.control_templates.fields.name);return c};this.getEndpointTypes=function(){return this.endpointTypes.indexed};this.getEndpoints=function(){return this.endpoints.indexed};this.controlList={};this.getControls=function(){angular.forEach(a.controls.indexed,function(b){var c=!1,d=!0;if(angular.isArray(a.endpointsSelected)&&0<a.endpointsSelected.length){var d=!1,k=b.fields.control_endpoint_id;angular.forEach(a.endpointsSelected,function(a){a==k&&(c=!0)})}else if("0"!==a.endpointSelected)d=
!1,a.endpointSelected==b.foreign.control_endpoint_id.id&&(c=!0);else if(angular.isArray(a.endpointTypesSelected)&&0<a.endpointTypesSelected.length){var d=!1,f=b.foreign.control_endpoints.fields.endpoint_type_id;angular.forEach(a.endpointTypesSelected,function(a){f==a&&(c=!0)})}c||d?a.controlList[b.id]=b:delete a.controlList[b.id]});return a.controlList};this.endpointList={};this.getEndpoints=function(){angular.forEach(a.endpoints.indexed,function(b){var c=!1,d=!0;if(angular.isArray(a.endpointTypesSelected)&&
0<a.endpointTypesSelected.length){var d=!1,k=b.fields.endpoint_type_id;angular.forEach(a.endpointTypesSelected,function(a){k==a&&(c=!0)})}c||d?a.endpointList[b.id]=b:delete a.endpointList[b.id]});return a.endpointList};this.clearEndpointTypes=function(){a.endpointTypesSelected=void 0};this.clearEndpoints=function(){a.endpointsSelected=void 0};this.addPanelControl=function(){c.addRow(a.newPanelControl);d.hide()};this.cancelAdd=function(){d.hide()}}];DevCtrl.EnumEditor={};
...
DevCtrl.Room.Ctrl).controller("RoomsCtrl",DevCtrl.Rooms.Ctrl).config(DevCtrl.stateConfig).run(["$rootScope",function(d){d.$on("$stateChangeStart",function(c,a,b,d,h){console.log("$stateChangeStart to "+a.to+"- fired when the transition begins. toState,toParams : \n",a,b)});d.$on("$stateChangeError",function(c,a,b,d,h){console.log("$stateChangeError - fired when an error occurs during transition.");console.log(arguments)});d.$on("$stateChangeSuccess",function(c,a,b,d,h){console.log("$stateChangeSuccess to "+
a.name+"- fired once the state transition is complete.")});d.$on("$viewContentLoaded",function(c){console.log("$viewContentLoaded - fired after dom rendered",c)});d.$on("$stateNotFound",function(c,a,b,d){console.log("$stateNotFound "+a.to+"  - fired when a state cannot be found by its name.");console.log(a,b,d)})}]);
