...
class Messenger {
    ...
    static broadcastControlValues(updates, fn, socket) {
        let controlsCollection = Messenger.mongodb.collection(Control_1.Control.tableStr);
        ...
        for (let update of updates) {
            ...
            if (update.status == "observed" && !update.ephemeral) {
                controlsCollection.updateOne({ _id: update.control_id }, { '$set': { value: update.value } }, (err, r) => {
                    if (err) {
    ...
    static disconnectSocket(socket) {
        if (socket["endpoint_id"]) {
            ...
            let col = Messenger.mongodb.collection(Endpoint_1.Endpoint.tableStr);
            col.updateOne({ _id: _id }, { '$set': { status: Endpoint_1.EndpointStatus.Offline } }, (err, r) => {
                if (err) {
     ...
     */
    static registerEndpoint(request, fn, socket) {
